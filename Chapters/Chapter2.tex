...
La estructura de MVB sigue el modelo OSI \cite{ISO7498-1}, que establece siete capas de abstracción para la transmisión de datos. A continuación se describen las capas inferiores, que son las más relevantes para el desarrollo de este trabajo.
...
La EDU-CIAA se utilizó para desarrollar el dispositivo generador de señal que permite probar el funcionamiento del dispositivo de captura. El diseño del generador se describe en la sección \ref{sec:generador}.
...
\subsection{Modulo analizador lógico}

El Saleae Logic 8 \cite{web:saleae} es un dispositivo de captura y análisis de señales digitales de alta velocidad y bajo costo. Se conecta a una computadora a través de USB y permite visualizar, medir y analizar señales digitales en tiempo real.
El dispositivo cuenta con 8 canales de entrada, cada uno de los cuales puede capturar señales de hasta 25 MHz.
Además, incluye un software intuitivo y fácil de usar que permite realizar decodificación de protocolos y análisis avanzado de señales.

En este trabajo se utilizó el Saleae Logic 8 en la fase de investigación, para realizar capturas del tráfico del bus MVB para su posterior análisis, como se menciona en la sección \ref{sec:capturas}.

El dispositivo de captura desarrollado también utiliza el Saleae Logic 8 para capturar el tráfico del bus en tiempo real, como se describe en la sección \ref{sec:hardware}.

%Para capturar el tráfico del bus MVB, en este trabajo se utilizó un dispositivo clon del Saleae Logic 8 \cite{web:saleae}, como el que se muestra en la figura~\ref{fig:saleae}. Este dispositivo e

%\begin{figure}[htbp]
%	\centering
%	\includegraphics[width=0.6\textwidth]{./Figures/clon-saleae.jpg}
%	\caption{El dispositivo clon del Saleae Logic 8}
%    \label{fig:saleae}
%\end{figure}
